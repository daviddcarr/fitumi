...
export default function CanvasBackground() {
    ...
    const createStroke = (): Stroke => {
        ...
        const freq = 0.5 + Math.random() * 1.0; // cycles over 0→1
        const amp = H * (0.1 + Math.random() * 0.15); // 10–25% of height

        const xPadding = W * 0.1;

        const phase = Math.random() * Math.PI * 2;
        const phaseH = ((Math.random() - 0.5) * 2) * (H / 2)
        ...
        for (let i = 0; i < N; i++) {
            ...
            const x = -xPadding + t * ( W + 2 * xPadding);
            const y = 
                (H / 2) + phaseH +
    ...
    const drawPartialStroke = (ctx: CanvasRenderingContext2D, s: Stroke, t: number) => {
